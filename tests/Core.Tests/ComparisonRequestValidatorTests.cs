...
namespace Core.Tests
{
    public class ComparisonRequestValidatorTests
    {
        ...
        [Fact]
        public void FileSizeLimitValidationOverLimitsFails()
        {
            var fileMock = new Mock<IFormFile>();
            fileMock.Setup(f => f.FileName).Returns("config.cfg");
            ...
            var result = validator.Validate(request);
            Assert.NotNull(result.Errors.First(error => error.PropertyName == nameof(request.SourceFile)));
            Assert.NotNull(result.Errors.First(error => error.PropertyName == nameof(request.TargetFile)));
        }
    }
}